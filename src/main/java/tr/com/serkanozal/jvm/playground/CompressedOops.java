...
public class CompressedOops {
	...
	public static class CompressedOopsInfo {
        ...
        public void setBaseAddressForObjectPointers(long baseAddressForObjectPointers) {
            this.baseAddressForObjectPointers = baseAddressForObjectPointers;
        }
        ...
        public void setBaseAddressForClassPointers(long baseAddressForClassPointers) {
            this.baseAddressForClassPointers = baseAddressForClassPointers;
        }
        ...
        public String toString() {
            if (enabled) {
                return 
                    "Compressed-Oops are enabled with " + 
                        "base-address: " + COMPRESSED_OOPS_INFO.baseAddressForObjectPointers + 
                        " and with " + 
                        "shift-size: " + COMPRESSED_OOPS_INFO.shiftSizeForObjectPointers;
            }
            else {
    ...
    private static CompressedOopsInfo findCompressedOopsInfo() {
        CompressedOopsInfo compressedOopsInfo = createCompressedOopsProvider().getCompressedOopsInfo();
        if (compressedOopsInfo != null) {
            return compressedOopsInfo;
        }
        else {
            return new DefaultCompressedOopsInfoProvider().getCompressedOopsInfo();
        }
    }
	...
	private static CompressedOopsInfoProvider createCompressedOopsProvider() {
	    if (JvmUtil.isHotspotJvm()) {
            if (JvmUtil.isJava_6()) {
                ...
                throw new IllegalStateException("Unsupported Java version: " + JvmUtil.JAVA_SPEC_VERSION);
            }    
	    }
	        ...
	        throw new IllegalStateException("Unsupported JVM: " + JvmUtil.JVM_NAME);
	    }
    }
	...
	private static class DefaultCompressedOopsInfoProvider implements CompressedOopsInfoProvider {
            ...
            if (oopSize != UNSAFE.addressSize()) {
                switch (oopSize) {
                    case JvmUtil.ADDRESSING_8_BYTE:
                        return new CompressedOopsInfo(0, log2p(JvmUtil.getObjectAlignment()));
                    case JvmUtil.ADDRESSING_16_BYTE:
                        return new CompressedOopsInfo(0, log2p(JvmUtil.getObjectAlignment()));
                    default:
                        throw new IllegalStateException("Unsupported address size for compressed reference shifting: " + oopSize); 
                }       
            }
            else {
	...
	private static class JRockitJvmCompressedOopsInfoProvider implements CompressedOopsInfoProvider {
	...
	private static class IbmJvmCompressedOopsInfoProvider implements CompressedOopsInfoProvider {
	...
	private static abstract class AbstractHotspotJvmCompressedOopsInfoProvider 
	    ...
	    protected static final int COMPRESSED_OOP_SHIFT_SIZE = log2p(JvmUtil.getObjectAlignment());
	    
	    protected static final HotspotJvmClassAddressFinder hotspotJvmClassAddressFinder = 
	            createClassAddressFinder();
	...
	private static abstract class AbstractHotspotJvmClassAddressFinder 
        ...
        protected static final long OBJECT_ARRAY_BASE_OFFSET = UNSAFE.arrayBaseOffset(Object[].class);
